...
def trim_format(time, verbosity = 1):
    if time.isdigit(): # it is all digits, see if its correct amount
        if not len(time) == 6: # 6 digits, this checks out
        ...
        for char in time:
            if char == ":":
                count += 1
        if count == 2:
            index_first = time.find(":")    # find the first :
            index_last = time.rfind(":")   # find the last :
            # capture substrings
            if len(time) == (index_last + 1):   #to prevent out of index error
                seconds_string = "00"   # there are no characters after last :
            else:
                seconds_string = time[(index_last +1):]   # add all characters after last :
            minutes_string = time[(index_first + 1):index_last]
            hours_string = time[:index_first]
            ...
            time = new_list[2] + ":" + new_list[1] + ":" + new_list[0]
...
def ffmpeg(source, dest, media_info, resolution, trim, verbosity = 1, codec = formats.copy, rate = 29.97):
    ...
    if resolution == resolution_type.hd_high or resolution_type.hd_low: # for now pad footage with lower than 1280x720 the same
    ...
    if codec == formats.imx50: # imx50
        ...
        line_1 = '-vf ' + '"' + scale + ':interl=0:in_color_matrix=bt709:out_color_matrix=bt601,' + pad + ',tinterlace=4:flags=vlpf" -c:v mpeg2video -pix_fmt yuv422p'

        # codec options, audio and flags
        line_2 = ' -b:v 50M -minrate 50M -maxrate 50M -bufsize 2M -rc_init_occupancy 2M -intra -flags +ildct+low_delay -intra_vlc 1 -non_linear_quant 1 -ps 1 -qmin 1 -qmax 3 -top 1 -dc 10 -c:a pcm_s24le -ar 48000 -d10_channelcount 4 -f mxf_d10'
        ...
        line_1 = hd_filters + "-vcodec mpeg2video -profile:v 0 -level:v 2 -b:v 50000k -maxrate 50000k -bufsize 3835k -minrate 50000k "
            ...
            if size_transcoded > 300:
                ...
                if os.path.isfile(dest):
                    ...
                    if dest_info['frames'] == source_info['frames']:
                        ...
                        if verbosity >= 2:
                            print "A " + str((dest_info['frames'] - source_info['frames'])) + " frame difference between the two"
                            print "ffmpeg: " + source
