...
from retrosupport.media import prores
from retrosupport.media import getinfo
from retrosupport.media import ffmbc
...
def re_encode(string):
    ...
    list = ["mkv", "flv", "mpg", "mpeg", "wmv"]

    # Process string to strip out extension
    string_index = string.rfind(".") + 1
    extension = string[string_index:]

    if extension in list:   # This particular extension is in our non avid supported list. Return True
        return True
    else:
        return False

# finds files with asset label in file name
...
def main():
    ...
    # Lets check and see if we need to make a ProRes version
    if re_encode(file_target):
        print "Looks like this file is not in an Avid supported format. Re-encoding into ProRes or MXF for " \
              "fast import. Depending on the resolution"

        # Get information about the file to determine format
        media_info = getinfo(file_target)
        # Get number of pixels in image
        pixels = int(media_info['width']) * int(media_info['height'])

        string_index = file_target.rfind('.') + 1   # Find the index to where the . before file extension is

        if pixels <= 415000:     # if the number of pixels are basically SD format
            ffmbc_target = file_target[:string_index] + "mxf"
            print "Video appears to be in standard def"
            print "Creating MXF file: " + ffmbc_target
            print "\nWARNING: The percentage done scale does not work with ffmbc"
            print "It will reach 100% and keep going....sigh"
            #ffmbc(file_target, ffmbc_target)
            print "/nFuck mxf's for now. Its not working. lets make prores/n"
            prores_target =  file_target[:string_index] + "mov"
            print "Creating ProRes file: " + prores_target
            prores(file_target, prores_target)
        else:    # we are going Prores here
            prores_target =  file_target[:string_index] + "mov"
            print "Video appears to be higher rez than standard def"
            print "Creating ProRes file: " + prores_target
            prores(file_target, prores_target)
